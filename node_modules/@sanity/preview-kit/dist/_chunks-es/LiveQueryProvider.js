...
import { isEqual, useQueryPerspective, defineStoreContext, useShouldPause } from "./hooks.js";
...
function getQueryCacheKey(query, params, perspective) {
...
function unsubscribe(queries, { payload }) {
  const key = getQueryCacheKey(payload.query, payload.params, payload.perspective), value = queries.get(key);
...
function LiveStoreProvider(props) {
  ...
  const perspective = useQueryPerspective(usePerspective(props.perspective || "drafts")), [client] = useState(() => {
  ...
  const { queries, snapshots, subscribe: subscribe2, update } = useLiveQueries(), context = useMemo(() => function(initialSnapshot, query, params, hookPerspective) {
    const effectivePerspective = hookPerspective || perspective, snapshotsKey = getQueryCacheKey(query, params, effectivePerspective);
    return { subscribe: (onStoreChange) => {
      const unsubscribe2 = subscribe2({
        query,
        params,
  ...
  }, [perspective, snapshots, subscribe2]), liveEvents = useLiveEvents(client);
  return /* @__PURE__ */ jsxs(defineStoreContext.Provider, { value: context, children: [
    children,
    [...queries.entries()].map(([key, { query, params, perspective: perspective2, listeners }]) => /* @__PURE__ */ jsx(
      QuerySubscription,
      {
        client,
        listeners,
