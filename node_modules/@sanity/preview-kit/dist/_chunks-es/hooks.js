import { createContext, useMemo, useState, useEffect, useSyncExternalStore, useContext, useCallback } from "react";
...
function useLiveQuery(initialData, query, queryParams, options) {
  const { isEqual: isEqual$1 = isEqual, perspective: perspectiveOption = null } = options || {}, defineStore = useContext(defineStoreContext), params = useQueryParams(queryParams), perspective = useQueryPerspective(perspectiveOption), noStore = useMemo(
    () => ({
      subscribe: (() => () => {
      }),
