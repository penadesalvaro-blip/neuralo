"use strict";
var react = require("react"), withSelector = require("use-sync-external-store/with-selector");
...
const defineStoreContext = react.createContext(null);
function useLiveQuery(initialData, query, queryParams, options) {
  const { isEqual: isEqual$1 = isEqual, perspective: perspectiveOption = null } = options || {}, defineStore = react.useContext(defineStoreContext), params = useQueryParams(queryParams), perspective = useQueryPerspective(perspectiveOption), noStore = react.useMemo(
    () => ({
      subscribe: (() => () => {
      }),
    ...
    () => defineStore?.(initialData, query, params, perspective) || {
      subscribe: (() => () => {
      }),
      getSnapshot: () => initialData
    },
  ...
  }), getServerSnapshot = react.useCallback(() => serverSnapshot, [serverSnapshot]), selector = react.useCallback((snapshot2) => snapshot2, []), snapshot = withSelector.useSyncExternalStoreWithSelector(
...
function useIsEnabled() {
  return react.useContext(defineStoreContext) !== null;
}
...
function useQueryPerspective(perspective) {
  const stringifiedPerspective = react.useMemo(() => JSON.stringify(perspective), [perspective]);
  return react.useMemo(
    () => JSON.parse(stringifiedPerspective),
...
exports.useQueryPerspective = useQueryPerspective;
