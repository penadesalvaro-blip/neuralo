...
function LiveStoreProvider(props) {
  ...
  const perspective = hooks.useQueryPerspective(usePerspective(props.perspective || "drafts")), [client] = react.useState(() => {
  ...
  const { queries, snapshots, subscribe: subscribe2, update } = useLiveQueries(), context = react.useMemo(() => function(initialSnapshot, query, params, hookPerspective) {
  ...
  return /* @__PURE__ */ jsxRuntime.jsxs(hooks.defineStoreContext.Provider, { value: context, children: [
    children,
    [...queries.entries()].map(([key, { query, params, perspective: perspective2, listeners }]) => /* @__PURE__ */ jsxRuntime.jsx(
      QuerySubscription,
      {
        client,
        listeners,
